...
# globably import scalar to avoid repeated loading
...
def metrics(document: str):
    ...
    # Generate metrics
    # Average word length
    story_length = df["transcription"].str.len()
    word_count = (df["transcription"].str.split()).str.len()
    df["avg_word_len"] = story_length / word_count
    ...
    # Read in the words we deemed important so we can get their count later
    words_path = Path('app/utils/complexity/words')
    with open((words_path / "emotional.txt"), "r") as file:
        emotional = set(file.read().split(","))

    with open((words_path / "imagistic.txt"), "r") as file:
        imagistic = set(file.read().split(","))

    with open((words_path / "cognitive.txt"), "r") as file:
        cognitive = set(file.read().split(","))
    ...
    tokened_story = re.sub(r'[^\w\s-]', '', document.strip().strip("/-0123456789").lower()).split()

    # lematization to look for appropriate words
    lemmatizer = WordNetLemmatizer()
    # initialize the vars to 0
    emotion = 0
    cognition = 0
    image = 0
    # loop over every word and if it's present in one of the sets,
    # increment the corresponding variable
    for word in tokened_story:
        lemma = lemmatizer.lemmatize(word, get_wordnet_pos(word))
        if lemma in emotional:
            emotion += 1
        elif lemma in cognitive:
            cognition += 1
        elif lemma in imagistic:
            image += 1

    df["emotional"] = emotion
    df["imagistic"] = image
...
async def squad_score(document: str, scaler):
    ...
    # Weight values
    awl = weights["avg_word_len"] * scaled[0] * range_scaler
    qn = weights["quotes_number"] * scaled[1] * range_scaler
    uw = weights["unique_words"] * scaled[2] * range_scaler
    an = weights["adj_num"] * scaled[3] * range_scaler
    em = weights["emotional"] * scaled[4] * range_scaler
    im = weights["imagistic"] * scaled[5] * range_scaler
    co = weights["cognitive"] * scaled[6] * range_scaler

    # Add all values
    squad_score = awl + qn + uw + an + em + im + co
